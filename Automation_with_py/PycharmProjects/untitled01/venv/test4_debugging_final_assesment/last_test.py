...
def get_file_lines(url):
    ...
    dictionary = {}
    with requests.get(url, stream=True) as x:
        lines = (line.decode('utf-8') for line in x.iter_lines())
        reader = csv.reader(lines)
        next(reader)
        for row in reader:
            dictionary[row[3]] = [row[0] + " " + row[1]]
...
def list_newer(start_date, data):
    employee = get_same_or_newer(start_date, data)
    for key in employee:
        print("Started on {}: {}".format(key.strftime("%b %d, %Y"), employee[key]))
